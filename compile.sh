...
pushd "${WORKDIR}" || exit 1
	...
	pushd opennms-full-assembly || exit 1
		"${WORKDIR}/bin/compile.pl" -Prun-expensive-tasks "${COMPILE_OPTIONS[@]}" "${SKIP_TESTS[@]}" -v install || update_github_status "${WORKDIR}" "failure" "$GITHUB_BUILD_CONTEXT" "assembly compile failed"
	popd || exit 1
