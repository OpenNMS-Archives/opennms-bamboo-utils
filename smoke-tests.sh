...
case "$SMOKE_TEST_API_VERSION" in
	"2"|"3"|"4"|"5"|"6"|"7"|"8"|"9")
		...
		cd opennms-source
			...
			cd smoke-test
				xvfb-run --wait=20 --server-args="-screen 0 1920x1080x24" --server-num=80 --auto-servernum --listen-tcp ../compile.pl -Dsurefire.rerunFailingTestsCount=2 -Dfailsafe.rerunFailingTestsCount=2 -Dorg.opennms.smoketest.logLevel=INFO -Dtest.fork.count=2 -Dorg.opennms.smoketest.docker=true -Dsmoke=true -t || exit 1
			cd ..
		cd ..
		;;
	*)
